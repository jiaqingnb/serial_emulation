...
#ifndef IDMAP_STRUCT_H
#define IDMAP_STRUCT_H
...
typedef struct DSU_NumMap_Len
{
	...
	UINT16 wTSLNumLen;			/*数据库中/TSL编号表的结构体的数量*/
...
/*TSL编号表*/
typedef struct DSU_TSLIdStruct
{
	UINT16   wIndexId;  /*索引编号*/
	UINT32  dwTslWirelessCommId;  /*TSL无线通信编号 */
	UINT32  dwTslWireCommId;  /*TSL有线通信编号 */
}DSU_TSL_NUM_STRU;
...
typedef struct DSU_StaticIDMap
{
	...
	DSU_TSL_NUM_STRU       	*pTSLNumStru;		/*TSL编号表结构体指针*/
...
typedef struct DSU_IdmapIndexStruct
{
	...
	UINT16* dsuCINumIndex;				/*联锁信息索引数组*/

	UINT16 ATSNUMINDEXSIZE;				/*ATS编号表索引数组空间大小*/
	UINT16* dsuATSNumIndex;				/*ATS信息索引数组*/
	...
	UINT16 TSLNUMINDEXSIZE;				/*TSL编号表索引数组空间大小*/
	UINT16* dsuTSLNumIndex;				/*TSL信息索引数组*/
