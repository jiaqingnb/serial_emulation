...
UINT32 dquGetTSLSecDevId(UINT16 dataPos);
...
UINT16 dquGetTSLIndex(UINT16 dataPos);
...
UINT16 dquGetTSLNumStruPos(DSU_IDMAP_INDEX_STRU* pDsuIdmapIndexStru, UINT16 devIndex);
...
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wZCNumLen);
}
UINT16 dquGetCINumLen(void)
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wCINumLen);
}
UINT16 dquGetATSNumLen(void)
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wATSNumLen);
}
UINT16 dquGetSignalNumLen(void)
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wSignalNumLen);
}
UINT16 dquGetTrackSecNumLen(void)
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wTrackSecNumLen);
}
...
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wStationNumLen);
}
UINT16 dquGetEmergBtnNumLen(void)
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wEmergBtnNumLen);
}
UINT16 dquGetBaliseNumLen(void)
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wBaliseNumLen);
}
UINT16 dquGetPSDoorNumLen(void)
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wPSDoorNumLen);
}
UINT16 dquGetVobcNumLen(void)
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wVobcNumLen);
}
UINT16 dquGetFPDoorNumLen(void)
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wFPDoorNumLen);
}
UINT16 dquGetDsuNumLen(void)
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wDsuNumLen);
}
UINT16 dquGetLogicSecNumLen(void)
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wLogicSecNumLen);
}
...
UINT16 dquGetAomSecNumLen(void)
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wAomSecNumLen);
}
...
UINT16 dquGetSpksSecNumLen(void)
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wSpksSecNumLen);
}
...
UINT16 dquGetGodSecNumLen(void)
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wGodSecNumLen);
}
...
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wStaSecNumLen);
}
...
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wPhySecNumLen);
}
/**************************************************
*函数功能：获取TSL互联互通编号长度信息
*输入参数：
*	无
*输出参数：
*	无
...
UINT16 dquGetTSLNumLen(void)
{
	return ((NULL == qds_dsuIdmapLenStru) ? 0u : qds_dsuIdmapLenStru->wTSLNumLen);
}
...
dquGetAomSecNumLen,dquGetSpksSecNumLen,dquGetGodSecNumLen,dquGetStaSecNumLen,dquGetPhySecNumLen,dquGetTSLNumLen };
/*根据设备数组位置，获取此设备编号*/
...
UINT32 dquGetTrackDevId(UINT16 dataPos)
{
	...
	{
		dwRetVal = qds_dsuStaticIdmapStru->pTrackSecNumStru[dataPos].dwTrackSecId[0];
	}
	return dwRetVal;
}
...
UINT32 dquGetAomSecDevId(UINT16 dataPos)
{
	UINT32 dwRetVal = 0u;
	if (dataPos < dquGetDataLen[QFUN_IDMAP_AOM_ID]())
	{
		dwRetVal = qds_dsuStaticIdmapStru->pAomSecNumStru[dataPos].dwAomSecId[0];
	}
	return dwRetVal;
}
...
*返回值：TSL互联互通编号
***************************************************/
UINT32 dquGetTSLDevId(UINT16 dataPos)
{
	UINT32 dwRetVal = 0u;
	if (dataPos < dquGetDataLen[QFUN_IDMAP_TSL_ID]())
	{
		dwRetVal = qds_dsuStaticIdmapStru->pTSLNumStru[dataPos].dwTslWirelessCommId;
	}
	return dwRetVal;
}

/*函数注册列表*/
const DQU_GETFUN_LIST_WrWD dquGetDevId = 
{ 
	NULL,
	dquGetZCDevId,
	dquGetCIDevId,
...
{
	UINT16 dwRetVal = 0;
	if ((NULL != qds_dsuStaticIdmapStru) && (NULL != qds_dsuStaticIdmapStru->pTSLNumStru))
	{
		if (dataPos < dquGetDataLen[QFUN_IDMAP_TSL_ID]())
		{
			dwRetVal = qds_dsuStaticIdmapStru->pTSLNumStru[dataPos].wIndexId;
		}
	}
...
/*函数注册列表*/
const DQU_GETFUN_LIST_WrW dquGetDevIndex = 
{ 
	NULL,
	dquGetZCIndex,
	dquGetCIIndex,
...
UINT16 dquGetTSLNumStruPos(DSU_IDMAP_INDEX_STRU* pDsuIdmapIndexStru, UINT16 devIndex)
{
	...
	if (NULL != pDsuIdmapIndexStru)
	{
		if (devIndex <= pDsuIdmapIndexStru->TSLNUMINDEXSIZE)
		{
			dataPos = pDsuIdmapIndexStru->dsuTSLNumIndex[devIndex];
		}
	}
	return dataPos;
}
...
/*函数注册列表*/
const DQU_GETFUN_LIST_IxWtW dquGetDevNumStruPos = 
{ 
	NULL,
	dquGetZCNumStruPos,
	dquGetCINumStruPos,
	dquGetATSNumStruPos,
	dquGetSignalNumStruPos,
	dquGetTrackSecNumStruPos,
	dquGetSwitchNumStruPos,
	dquGetStationNumStruPos,
	dquGetEmergBtnStruPos,
	dquGetBaliseNumStruPos,
	dquGetPSDoorNumStruPos,
	dquGetVobcNumStruPos,
	dquGetFPDoorNumStruPos,
	dquGetDsuNumStruPos,
	dquGetLogicSecNumStruPos,
	dquGetAomSecNumStruPos,
	dquGetSpksSecNumStruPos,
	dquGetGodSecNumStruPos,
	dquGetStaSecNumStruPos,
	dquGetPhySecNumStruPos,
	dquGetTSLNumStruPos
};
...
UINT8 dquQueryTrackIdOffset(UINT16 linkId, UINT32 linkOffset, UINT32 *pTrackSecId, UINT32 *pTrackOffset)
{
	...
	UINT32 dwCurTrackId = 0u, dwCurTrackLen = 0u, dwTrackSumLen = 0u;

	if ((NULL != pTrackSecId) && (NULL != pTrackOffset))
	{
		if (DQU_IDMAP_INIT_TRUE == qds_IsInitIdMapStruPtr)
		{
			/*查找轨道区段*/
			for (wPos = 0u; wPos < qds_dsuIdmapLenStru->wTrackSecNumLen; wPos++)
			{
			...
			if ((1u == bFound) && (NULL != pTrackSecStru))
			{
				for (wPos = 0u; wPos < pTrackSecStru->wTrackNum; wPos++)
				{
...
{
	UINT16 wTrackIndex = 0u, wPos = 0u;
	UINT8 bRetVal = 0u;
	UINT8 bFound = 0u;
	DSU_TRACKSEC_NUM_STRU *pTrackSecStru = NULL;
	UINT32 dwCurTrackId = 0u, dwCurTrackLen = 0u, dwTrackSumLen = 0u;

	if ((NULL != pLinkId) && (NULL != pLinkOffset))
	{
		if (DQU_IDMAP_INIT_TRUE == qds_IsInitIdMapStruPtr)
		{
			/*找到轨道区段*/
			...
			{
				pTrackSecStru = qds_dsuStaticIdmapStru->pTrackSecNumStru + wPos;
				for (wTrackIndex = 0u; wTrackIndex < pTrackSecStru->wTrackNum; wTrackIndex++)
				{
...
UINT8 dquQueryInVobcDevIndex(UINT32 devId, UINT16 *pInDevIndex)
{
	...
	if (NULL != pInDevIndex)
	{
		*pInDevIndex = 0u;	/*增加对输出函数的初始化操作，防止未对该变量赋值时返回随机值*/
	...
	if (NULL != arOutDevIds)
	{
		if (DQU_IDMAP_INIT_TRUE == qds_IsInitIdMapStruPtr)
		{
			...
			if (0xffffu != dataPos)
			{
				...
				{
					CommonMemCpy(arOutDevIds, sizeof(UINT32) * 2u, dquVobcDevIdArrayData, sizeof(UINT32) * 2u);

					if ((0u != arOutDevIds[0]) || (0u != arOutDevIds[1]))
					{
						bRetVal = 1u;
					}
	...
	if (NULL != arOutDevIds)
	{
		if (DQU_IDMAP_INIT_TRUE == qds_IsInitIdMapStruPtr)
		{
			...
			if (0xffffu != dataPos)
			{
				...
				{
					CommonMemCpy(arOutDevIds, sizeof(UINT32) * 2u, dquQueryOutAOMDevIdData, sizeof(UINT32) * 2u);
...
*修改：by yt 2021年9月11日 修改返回值参数类型不正确的问题
***************************************************/
static UINT16 dquBaliseLineId(UINT16 dataPos)
{
	UINT16 wRetVal = 0u;
	if ((NULL != qds_dsuStaticIdmapStru) && (NULL != qds_dsuStaticIdmapStru->pBaliseNumStru))
	{
		if (dataPos < dquGetDataLen[QFUN_IDMAP_BALISE_ID]())
		{
			wRetVal = qds_dsuStaticIdmapStru->pBaliseNumStru[dataPos].wLindId;
		}
	}
