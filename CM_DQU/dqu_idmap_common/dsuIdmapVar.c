...
static UINT16 g_dsuIdmapDataArrayLen[DFS_DATA_INDEX_NUM] = { 0u };/*用于存储编号对照表中各项数据项数组的个数*/
static UINT16 g_dsuIdmapIndexArrayLen[DFS_DATA_INDEX_NUM] = { 0u };/*用于存储编号对照表中各项索引数组的个数*/
...
{
	UINT8 bRetVal = 0u;
	if ((DFS_DATA_INDEX_NUM > wDataID) && (NULL != pStruDataLen))
	{
...
{
	UINT8 bRetVal = 0u;
	if ((DFS_DATA_INDEX_NUM > wDataID) && (NULL != pDataIndexLen))
	{
