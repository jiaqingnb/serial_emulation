...
*				根据数据解耦修改了电子地图数据中的部分结构体 by lyk 20220307
************************************************************************/

#ifndef QUERY_STRUCT_H
#define QUERY_STRUCT_H
...
#define MAX_VIRTUAL_SWITCH_NUM 5u  /* 单个多开道岔拆分虚拟道岔最大数量*/

/********坡度包含link个数，坡度结构体专用*******/
#define GRADE_INCLUDE_LINK_NUM			10u		/*坡度中间link最大值*/
#define ROUTE_ACCESS_LINK_POINT_NUM		10u		/*进路接近区段道岔个数最大值*/
#define MAX_AXLESGMT_NUM_IN_ROUTE		20u		/*进路包含最大计轴区段*/

#define COLSE_BALISE_MAX_NUM			16u		/*应答器相邻应答器最大数量*/ /*根据互联互通需求相邻应答器数量变为16个，20160315，by guojian.hou*/

/********中间link个数，计轴区段和逻辑区段结构体专用*******/
#define MIDDLE_LINK_MAX_NUM				3u		/*150608 中间Link最大数量*/ /*2-->3 计轴区段包含道岔为4，中间Link为3 by qxt 20160915 */
#define SPKS_INCLUDE_LINK_NUM			500u	/*SPKS 包含link最大值 由100增加到200  m by sds 2019-5-17*/

#define	DSU_TRUE		0x55u 
#define	DSU_FALSE		0xaau

#define DSU_NULL_8		0xffu				/*无效定义宏*/  
#define DSU_NULL_16		0xffffu				/*无效定义宏*/      
#define DSU_NULL_32		0xffffffffu			/*无效定义宏*/  

/*最大Link数目宏定义*/
#define MAX_LINK_NUM0	200u

/*每个link里面最多包含的逻辑区段数量*/
#define MAX_LOGIC_SGMT_OF_LINK_NUM		20u

/*每个逻辑区段所属最多的进路数量*/
#define LOGIC_SGMT_OF_ROUTE_MAX_NUM		128u

/*单个ZC分界点相关共管区域的个数*/
#define RELATE_ZC_AREA_NUM				8u		/*2013.8.22，根据ZC应用需求，增加ZC分界点最大共管区域个数*/

/*link索引表包含最大静态限速数组大小*/
#define LINK_MAX_LIMIT_SPD				32u		/*10-->32 qxt 20170821*/

/*link上需关联设备中相应设备最大数量，目前以以Link上应答器的数量为最大值 add by qxt 20170815*/
#define LINK_MAX_DEVICE_NUM				200u
...
#define MAX_BLOC_ACCESS_LINK_NUM		30u		/*点式接近区段数据最大数量*/
#define MAX_CBTC_ACCESS_LINK_NUM		30u		/*CBTC接近区段数据最大数量*/
#define MAX_BLOC_TRG_AREA_NUM			30u		/*点式触发区段数据最大数量*/
#define MAX_CBTC_TRG_AREA_NUM			30u		/*CBTC触发区段数据最大数量*/

/*根据感知点式MA需求添加 wyd 20220214*/
#define MAX_YELLOW_ROUTE_NUM			36u		/*信号机点黄灯对应的进路数量最大值*/
#define MAX_PROTECTLINK_POINT_NUM		16u		/*单个保护区段内最大道岔数量（按当前数据结构，包含4个计轴区段，每个计轴区段包含4个道岔时数量最大）*/

#define MAX_ROUTE_NUM					37u		/*进路数量最大值*/
#define MAX_ADJACENT_LOGICSGMT_NUM		5u		/*相邻计轴区段最大值*/

#define MAX_LOGIC_SGMT_OF_ROUTE_NUM		255u	/*每条进路中最大的逻辑区段数量*/
#define MAX_LOGIC_SGMT_OF_PROTSGMT_NUM	255u	/*每条保护区段中最大的逻辑区段数量*/

#define DSU_MAX_LINK_BETWEEN 400u
#define DSU_MAX_STATION		 20u
...
typedef struct DSU_Data_Len
{
	...
	UINT16 wEvacuationSideLen;	    /*疏散侧表长度，依据互联互通数据结构V28.0.0.1添加，add by hzj 20230301*/
}DSU_DATA_LEN_STRU;
...
typedef struct DSU_LinkStruct
{
	...
	UINT16  wManageTIOC;            /*所属物理TIOC区域编号 add by xly 2022-3-2*/
	...
	UINT8 wLogicDirChanged;			/*link 逻辑方向变化点*/
	UINT8 wLinkNeighborType;		/*link邻线属性*/
	UINT8 wTunAttribute;			/*Link隧道属性，依据互联互通数据结构V12.0.0添加，add by lmy 20180312*/

	UINT16 LogicSgmtNum;								/*对应的逻辑区段数量*/
	UINT16 LogicSgmtIdBuff[MAX_LOGIC_SGMT_OF_LINK_NUM];	/*逻辑区段数组*/

	UINT8 EmapSameDirSignalNum;			/*在此link上和电子地图同方向的信号机数量*/
	UINT16 EmapSameDirSignalIdBuff[10];	/*在此link上和电子地图同方向的信号机列表*/
	UINT8 EmapConvDirSignalNum;			/*在此link上和电子地图反方向的信号机数量*/
	UINT16 EmapConvDirSignalIdBuff[10];	/*在此link上和电子地图反方向的信号机列表*/
...
typedef struct DSU_SignalStruct
{
	...
	UINT16  wPositionLinkId;						/*信号机真实位置所处link编号*/
	UINT32  dwPositionLinkOfst;						/*信号机真实位置所处link偏移量(cm)*/
	...
	UINT16 wMainSignalId;							/*关联主信号机ID*/
													/* 以下项在数据代码分离项目中，从数据文件中删除，它的值需要动态计算---严广学 */
	UINT16  wManageZC;								/*所属ZC区域编号*/
	UINT16  wManagePhysicalCI;						/*所属物理CI区域编号*/
	UINT16  wManageLogicCI;							/*所属逻辑CI区域编号*/
	UINT16	wGreenRouteId;							/*信号机点绿灯时，对应的进路ID（复示信号机填关联主信号机的）*/
	UINT8	wYellowRouteNum;						/*信号机点黄灯时对应的进路数量（复示信号机填关联主信号机的）*/
	UINT16	wYellowRouteId[MAX_YELLOW_ROUTE_NUM];	/*信号机点黄灯时，对应的进路ID（复示信号机填关联主信号机的）*/
	UINT16  wAllRouteNum;							/*始端为该信号机的所有进路数量*/
	UINT16  wAllRouteIdBuff[MAX_ROUTE_NUM];			/*始端为该信号机的所有进路id数组*/
}DSU_SIGNAL_STRU;
...
typedef struct DSU_AxleStruct
{
	...
	UINT16  wLinkId;						/*计轴所处Lnk*/
	UINT32  wLnkOff;						/*计轴所处Off*/
}DSU_AXLE_STRU;
...
typedef struct DSU_ScreenStruct
{
	...
	UINT8   wLittlePSDNUM;				 		/*此站台小屏蔽门的数量 ADD by lpw 20220520*/
...
typedef struct DSU_StopPointStruct
{
	...
	UINT16  wFirstPSDID;				 	/*第一个客室门对应小屏蔽门ID--ADD by lpw*/
	UINT16  wLastPSDID;				 		/*最后一个客室门对应小屏蔽门ID--ADD by lpw*/
	UINT16  wFirstDOORID;				 	/*开门范围对应起始客室门编号--ADD by lpw*/
	UINT16  wLastDOORID;				 	/*开门范围对应终止客室门编号--ADD by lpw*/
...
typedef struct DSU_AxleSgmtStruct
{
	...
	UINT16  wLogicSgmtId[50];						/*计轴区段包含的逻辑区段的编号*/
...
typedef struct DSU_LogicSgmtStruct
{
	...
	/* 以下项在数据代码分离项目中，从数据文件中删除，它的值需要动态计算---严广学 */
	UINT8   wAxleSgmtNum;						/*所属计轴区段数量*/
	UINT16  wAxleSgmtId[MAX_ADJACENT_LOGICSGMT_NUM];		/*所属计轴区段编号*/
	...
	UINT8	wOrgnLogicSgmtNum;								/*起点相邻逻辑区段数量*/
	UINT16	wOrgnLogicSgmtId[MAX_ADJACENT_LOGICSGMT_NUM];	/*起点相邻逻辑区段ID*/
	UINT8	wTmnlLogicSgmtNum;								/*终点相邻逻辑区段数量*/
	UINT16	wTmnlLogicSgmtId[MAX_ADJACENT_LOGICSGMT_NUM];	/*终点相邻逻辑区段ID*/
	UINT8	wDirChangedTag;						/*逻辑方向变化点，0x55终点相邻终点，0xAA起点相邻起点*/

	UINT8   RouteNum;											/*所属进路数量*/
	UINT16  RouteIdBuff[LOGIC_SGMT_OF_ROUTE_MAX_NUM];			/*所属进路id数组*/

	UINT8   AccessSgmtNum;										/*所属接近区段数量*/
	UINT16  AccessSgmtIdBuff[LOGIC_SGMT_OF_ROUTE_MAX_NUM];		/*所属接近区段id数组*/
...
typedef struct DSU_RouteStruct
{
	...
	UINT16  wBlocAccessLinkNum;					/*点式接近区段数目*/
	UINT16  wBlocAccessLinkIdBuff[MAX_BLOC_ACCESS_LINK_NUM];/*点式接近区段编号数组*/
	UINT16  wAccessLinkNum;						/*CBTC接近区段数目*/
	UINT16  wAccessLinkIdBuff[MAX_CBTC_ACCESS_LINK_NUM];				/*CBTC接近区段编号数组*/
	...
	UINT32	wStartLocOfst;						/*进路起点offset*/
	UINT8	wStartLocDir;						/*进路起点方向*/
	UINT32	wEndLocOfst;						/*进路终点offset*/
	UINT8	wEndLocDir;							/*进路终点方向*/

	/*根据感知点式MA需求添加，wyd 20220215*/
	/*下面的“反位保护区段”指与进路方向相同开向的道岔全在反位的保护区段，用于计算点式MA终点*/
	UINT32	dwLength;											/*进路长度*/
	UINT16	wMaEndLkId;											/*在反位保护区段上，延伸最短保护区段长度的Link编号*/
	UINT32	dwMaEndLkOfst;										/*在反位保护区段上，延伸最短保护区段长度的Link偏移量*/
	UINT16	wMaEndSidePointNum;									/*反位保护区段道岔数量*/
	UINT16	wMaEndSidePointId[MAX_PROTECTLINK_POINT_NUM];		/*反位保护区段道岔编号*/
	UINT16	wMaEndSidePointStatus[MAX_PROTECTLINK_POINT_NUM];	/*反位保护区段道岔状态*/

	/*wyd 20220309*/
	UINT16	wIncludeLogicSgmtNum;					/*包含的逻辑区段数目*/
	UINT16	wLogicSgmtId[50];						/*逻辑区段编号*/
	UINT8	wLogicSgmtDir[50];						/*逻辑区段应该的方向*/
}DSU_ROUTE_STRU;
...
typedef struct DSU_CBTCAccessLinkStruct
{
	...
	/******************************动态计算******************************************/
	UINT16  BelongRouteNum;						/*这个接近区段属于的进路的数量*/
	UINT16  BelongRouteIdBuff[128];				/*这个接近区段属于的进路的列表*/
...
typedef struct DSU_TrainLineStruct
{
	...
	UINT8  wIncludePointNum;						/*包含道岔数目*/
	UINT16  wPointId[16];							/*道岔编号*/
	UINT8  wPointStatus[16];						/*道岔状态*/
}DSU_TRAIN_LINE_STRU;
...
{
	UINT16  wId;											/*索引编号*/
	UINT16  wStartPlartformId;				/*无人折返出发停车区域编号*/
	UINT16  wARInStopPointId;				/*进入点对应停车点编号*/
	UINT16  wRevertPointId;				    /*换端点对应停车点编号*/
	UINT16  wEndPlartformId;				/*退出停车区域编号*/
	UINT16  wAROutStopPointId;				/*退出点对应停车点编号*/
...
/*疏散侧表结构*/
typedef struct DSU_EVACUATIONSIDE_Struct
{
	UINT16	wId;						/*索引编号*/
	UINT16  wOrgnLinkId;                /*解锁区域起点Link编号*/
	UINT32  dwOrgnLinkOfst;             /*解锁区域起点Link偏移量*/
	UINT8   wOrgnLinkDir;				/*解锁区域起点 查询终点逻辑方向*/
	UINT16  wTmnlLinkId;                /*解锁区域终点 Link编号*/
	UINT32  dwTmnlLinkOfst;             /*解锁区域终点Link偏移量*/
	UINT8   wTmnlLinkDir;				/*解锁区域终点查询起点逻辑方向*/
	UINT8   wModeOfOpenDoor;			/*客室门紧急解锁开门方式*/
	UINT8   wCINum;                     /*解锁区域所属物理CI区域数量*/
	UINT8   wCIId[3];                   /*解锁区域所属物理CI区域编号*/
	UINT8   wLinkLine;                  /*解锁区域所属线路上下行*/
	UINT32  wEvacuationLenth;			/*解锁区域长度(cm)*/
	UINT16  wLinkNum;			        /*起点到终点包含link数目（包含起点和终点）*/
	UINT16  wLinkId[20];			    /*包含的link编号*/
}DSU_EVACUATION_SIDE_STRU;
...
{
	UINT16  wId;						/*索引编号*/
	UINT8	wIncLinkNum;				/*包含link数量*/
	UINT16	wIncLinkId[20];				/*包含link数组*/
...
typedef struct DSU_SpksButton
{
	...
	UINT16 wRelatePhysicalSgmtId[100];          /*物理区段ID编号*/
...
typedef struct DSU_DepotGate
{
	...
	UINT16 wBPhysicalSgmtId;					/*车库门对应B库线物理区段ID*/
...
typedef struct DSU_StaticHead/*存放静态数据各个数据结构的头指针*/
{
	...
	DSU_EXIT_ROUTE_STRU *pExitRouteStru;				/*退出CBTC区域信息结构体指针*/
	DSU_TRAIN_LINE_STRU *pTrainLineStru;				/*非CBTC区域信息结构体指针*/
	...
	DSU_EVACUATION_SIDE_STRU *pEvacuationSideStru; /*疏散侧信息结构体指针*/
...
typedef struct DSU_EmapIndexStruct
{
	...
	UINT16 EVASIDEINDEXNUM;	        /*疏散侧索引数组大小*/
	UINT16 *dsuEvacuationSideIndex;	/*疏散侧索引数组*/
