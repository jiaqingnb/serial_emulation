...
*			UINT8 data[]                        纯数据
*			struc_Unify_Info *pUnifyInfoForOutput	SFP结构体
*输入出参:	无
*输出参数:	无
*返回值:	插入成功返回当前指针,否则返回错误(插入操作返回空即为错)
*/
static struc_OutputDataBuff* Insert_OutputDataBuff(struc_OutputDataBuff *pLastNode, struc_OutputDataBuff *pCurNode, UINT32 DevName, UINT8 data_status, UINT16 data_len, UINT8 data[], struc_Unify_Info *pUnifyInfoForOutput)
{
...
*/
static struc_OutputDataBuff* Delete_OutputDataBuff(struc_OutputDataBuff *pLastNode,struc_OutputDataBuff *pCurNode, struc_Unify_Info *pUnifyInfoForOutput)
{
    ...
    if (PushStack(&pUnifyInfoForOutput->OutputStack,pCurNode) != 1)
    {
        /*记录记录，堆栈满，压栈错误*/
#ifdef PRTCL_RECORD_ERR_ASC
        PrintFiles(pUnifyInfoForOutput->RecordArray, pUnifyInfoForOutput->RecordArraySize,"E:");
...
/*
*函数名:	DeQueue_OutputData
*功能描述:	从队列中取出数据放入出口结构体元素中
*输入参数:	struc_Unify_Info *pUnifyInfoForOutput	SFP结构体
*输入出参:	无
*输出参数:	struc_AppOutput* pOut	应用的输出的1个对象的数据,指针变量
*返回值:	1:成功;0失败
*/
static INT8 DeQueue_OutputData(struc_AppOutput *pOut, struc_Unify_Info *pUnifyInfoForOutput)
{
...
*返回值:	1:成功;-1失败
*/
static INT8 Queue2Buff(struc_Unify_Info *pUnifyInfoForOutput)
{
	...
	IsQueueEmpty = DeQueue_OutputData(&pCurProc_QueueItem, pUnifyInfoForOutput);
            ...
            if(pCurNode != NULL)
            {
                ...
                {/*数据队列中节点设备大于数据缓冲链表节点设备*/
					...
					pCurNode = Insert_OutputDataBuff(pLastNode, pCurNode, pCurProc_QueueItem.DestDevName,
						NEED2SEND, pCurProc_QueueItem.AppDataLen, pCurProc_QueueItem.chAppData, pUnifyInfoForOutput);
                    if( pCurNode  == NULL)
                    {
                        /*添加失败*/
#ifdef PRTCL_RECORD_ERR_ASC
...
*			struc_Unify_Info *pUnifyInfoForOutput	SFP结构体
*输入出参:	UINT8 *chCurProcItemSrc	当前要处理的元素的来源
*			struc_Lnk_Mngr **pCurProc_Lnk				   当前链路
*			struc_OutputDataBuff **pLastDataBuffItem;      表示当前处理的数据存储单元的指针
*			struc_OutputDataBuff **pCurDataBuffItem;       表示上一次处理的数据存储单元的指针
*输出参数:	无
*返回值:	成功返回非负值（当有节点要处理时返回1，无节点要处理时返回0）,失败返回-1  
*/
static INT8 GetNextItem2Proc(UINT8 NeedToInit, UINT8 *chCurProcItemSrc, struc_Lnk_Mngr **pCurProc_Lnk, struc_OutputDataBuff **pLastDataBuffItem, struc_OutputDataBuff **pCurDataBuffItem, struc_Unify_Info *pUnifyInfoForOutput)
{
    UINT8 temp;
    INT8 ret;
    ...
    if((NULL==(*pCurDataBuffItem)) && (NULL==(*pCurProc_Lnk)))/*当前处理的链路管理表的节点并且当前要处理的数据缓冲链表的指针也为空*/
    {
        ret = 0;
        return ret;/*已经没有有效的处理对象,返回0*/        
    }		
    else if(NULL==(*pCurProc_Lnk))/*链路管理表中指针为空*/
    ...
    {/*要处理的数据来自链路管理表,说明要在这里插入一个节点用来存储此节点的数据*/
		...
		*pCurDataBuffItem = Insert_OutputDataBuff(*pLastDataBuffItem, *pCurDataBuffItem,
			(*pCurProc_Lnk)->Lnk_Info.nDestDevName, SENT, 1, &temp, pUnifyInfoForOutput);
...
*			struc_Unify_Info *pUnifyInfoForOutput	SFP结构体
*输入出参:	UINT32 *nNumsPrtclSnd				数据下标
*输出参数:	无
*返回值:	1:成功;0失败
*/
static INT8 BuildFrame(UINT8 frametype, struc_OutputDataBuff *pSrcData, struc_Lnk_Mngr *pSrcLnk, UINT32 *nNumsPrtclSnd, struc_Unify_Info *pUnifyInfoForOutput)
{
    UINT16 temp_short;
    UINT16 temp_SendLen;
    UINT32 temp_long;
...
#ifdef DUAL_CRC
    temp_short = 27 + 1 + 1 + 1 + 2 + temp_SendLen;
#else
	...
	msgBuff = pUnifyInfoForOutput->PrtclSndBuff+(*nNumsPrtclSnd);

#ifdef DUAL_CRC
    temp_short = 25 + temp_SendLen;
#else
...
#ifdef DUAL_CRC
        if (0u == pUnifyInfoForOutput->CpuId) /*CPU0 第一CRC位置赋值*/
        {
            pUnifyInfoForOutput->PrtclSndBuff[(*nNumsPrtclSnd)++] = temp_char[0];
            pUnifyInfoForOutput->PrtclSndBuff[(*nNumsPrtclSnd)++] = temp_char[1];
            pUnifyInfoForOutput->PrtclSndBuff[(*nNumsPrtclSnd)++] = temp_char[2];
            ...
            pUnifyInfoForOutput->PrtclSndBuff[(*nNumsPrtclSnd)++] = 0u;
            pUnifyInfoForOutput->PrtclSndBuff[(*nNumsPrtclSnd)++] = 0u;
            pUnifyInfoForOutput->PrtclSndBuff[(*nNumsPrtclSnd)++] = 0u;
            pUnifyInfoForOutput->PrtclSndBuff[(*nNumsPrtclSnd)++] = 0u;
        }
...
#else
#ifdef SWAP_CRC
        tmplen = ShortFromChar(&pUnifyInfoForOutput->SwapCrcs[0]); /*获取2字节总长度*/
        LongToChar(temp_long, &pUnifyInfoForOutput->SwapCrcs[2u + tmplen]); /*追加CRC*/
        ShortToChar(tmplen + 4u, &pUnifyInfoForOutput->SwapCrcs[0]); /*设置2字节总长度*/
...
*返回值:	1:成功;0失败
*/
static INT8 FillLnkMngr_into_PrtclSnd(struc_Lnk_Mngr *CurLnk, UINT32 *nNumsPrtclSnd, struc_Unify_Info *pUnifyInfoForOutput)
{
    UINT8  temp_char[4];
    UINT32 CurFrameStartIndex;
    INT8 ret;
...
UINT8 ProtclOutput(struc_Unify_Info *pUnifyInfo)/* %RELAX<MISRA_8_10> */
{
    ...
    UINT32 LocalDevName;
    UINT8 ret = 0;
	UINT8 chCurProcItemSrc = 0;	/*当前要处理的元素的来源 ITEMSRC_BOTH (数据队列和链路管理表) 0x17;ITEMSRC_DATAQUEUE (数据队列) 0x33;ITEMSRC_LNKMNGR(链路管理表) 0x55*/
	struc_Lnk_Mngr *pCurProc_Lnk = NULL;	/*链路管理表中当前处理的的链路指针,这里是指针，不是实体*/
	struc_OutputDataBuff *pLastDataBuffItem = NULL;	/*表示上一次处理的数据存储单元的指针*/
	struc_OutputDataBuff *pCurDataBuffItem = NULL;	/*表示当前处理的数据存储单元的指针*/
	UINT32 nNumsPrtclSnd = 0;	/*协议输出的数据的个数(字节为单位)*/
	struc_Unify_Info *pUnifyInfoForOutput = pUnifyInfo;	/*SFP结构体*/
...
#ifdef SWAP_CRC
    ShortToChar(0u, &pUnifyInfoForOutput->SwapCrcs[0]); /*清空*/
#endif
    ...
    if( Queue2Buff(pUnifyInfoForOutput) == -1)/*填充失败，返回错误*/
    {
        ret = 1;
        return ret;
    }
	...
	ResultofGet = GetNextItem2Proc(blNeedToInit, &chCurProcItemSrc, &pCurProc_Lnk, &pLastDataBuffItem, &pCurDataBuffItem, pUnifyInfoForOutput);
            ...
            {  
                ...
                case ITEMSRC_DATABUFF:/*只在数据缓冲链表出现，在链路管理表中无此对象*/
                        ...
                        {/*如果建链合法*/      				  
                            if(Role == ROLE_SPON)/*本方为发起方*/
                            {
                                ...
                                {                  
									...
									BuildFrame(FRAME_RFC, pCurDataBuffItem, pCurProc_Lnk, &nNumsPrtclSnd, pUnifyInfoForOutput);/*组织RFC帧*/
                    ...
                    else/*不是最新数据,删除这个节点*/
                    {
						...
						pCurDataBuffItem = Delete_OutputDataBuff(pLastDataBuffItem, pCurDataBuffItem, pUnifyInfoForOutput);
                ...
                case ITEMSRC_BOTH:		/*当两者都有时,需要根据链路状况和数据状态综合考虑*/
                    ...
                    case STATUS_DATA:/*数据状态*/
                        ...
                        case SENT:/*已经发送*/
                            /*组织IDLE帧*/
							BuildFrame(FRAME_IDLE, pCurDataBuffItem, pCurProc_Lnk, &nNumsPrtclSnd, pUnifyInfoForOutput);
                            break;
                        case NEED2SEND:
                            /*根据当前数据存储空间中数据组建DATA帧；*/
							BuildFrame(FRAME_DATA, pCurDataBuffItem, pCurProc_Lnk, &nNumsPrtclSnd, pUnifyInfoForOutput);
                    ...
                    case STATUS_HALT:/*通信中断状态*/
                        ...
                        {                
                            ...
                            pCurProc_Lnk->Lnk_Info.RfcSN = pUnifyInfoForOutput->RadamSN;
                            /*组织RFC帧*/
							BuildFrame(FRAME_RFC, pCurDataBuffItem, pCurProc_Lnk, &nNumsPrtclSnd, pUnifyInfoForOutput);
                        }
                        else
                        {}
      				    break;
                    case STATUS_PRECON:/*收到RFC准备回ACK的状态*/
                        if(pCurProc_Lnk->Lnk_Info.chRole == ROLE_FLW)/*本方为跟随方*/
                        {
                            ...
                            /*根据RFC组建ACK帧；*/
							BuildFrame(FRAME_ACK, pCurDataBuffItem, pCurProc_Lnk, &nNumsPrtclSnd, pUnifyInfoForOutput);
                        }
                        else
                        {
                            /*本方为发起方，不做处理*/   
    ...
    while (CurFillLnk != NULL)
    {
        ...
        /*添加特定链路到数组中*/		
		FillLnkMngr_into_PrtclSnd(CurFillLnk, &nNumsPrtclSnd, pUnifyInfoForOutput);
        /*记录记录,链路状态信息*/	
        /*当前处理的链路管理表的节点后移一个节点*/
...
*输入参数:	UINT8 swapData[] 交互数据(2字节长度+N个[4字节CRC])
*           const UINT16 swapDataMaxLen 交互数据最大长度
*           struc_Unify_Info* sfpInfo SFP结构体变量
*输入出参:	无
*输出参数:	无
*返回值:	无
*/
void SfpGetSwapData(UINT8 swapData[], const UINT16 swapDataMaxLen, struc_Unify_Info* sfpInfo)
{
    ...
    if ((NULL != swapData) && (NULL != sfpInfo) && (2u < swapDataMaxLen)) /*参数检查*/
    {
        record = sfpInfo->RecordArray;
        recordSize = sfpInfo->RecordArraySize;
        PrintRecArrayByte(record, recordSize, 0xE1u);

        ShortToChar(0u, &swapData[0]); /*长度清零*/
        swapDataLen += 2u;

#ifdef DUAL_CRC
        /*PrtclSndBuff格式:4字节总长度(应用和链路)+4字节输出应用长度+N个[2字节长度+1字节协议类型+2字节目的地址+2字节预留+网络数据]+链路数据*/
        allLen = LongFromChar(&sfpInfo->PrtclSndBuff[4]);
        if (0u < allLen) /*有输出*/
        {
            outAppData = &sfpInfo->PrtclSndBuff[8]; /*应用数据开始地址*/
            while (outAppIdx < allLen) /*遍历输出应用数据*/
            {
                pkgLen = 2u + ShortFromChar(&outAppData[outAppIdx]);
                if ((34u <= pkgLen) && ((outAppIdx + pkgLen) <= allLen)) /*包长度大于等于(7字节头+27字节帧格式),且当前输出应用位置加长度小于等于输出应用长度,合法*/
                {
                    if ((swapDataLen + 4u) <= swapDataMaxLen) /*有空间*/
                    {
                        if (0u == sfpInfo->CpuId) /*复制CPU0 CRC*/
                        {
                            CommonMemCpy(&swapData[swapDataLen], 4u, &outAppData[outAppIdx + pkgLen - 8u], 4u);
                        }
                        else /*复制CPU1 CRC*/
                        {
                            CommonMemCpy(&swapData[swapDataLen], 4u, &outAppData[outAppIdx + pkgLen - 4u], 4u);
                        }
...
#else
#ifdef SWAP_CRC
        swapDataLen = 2u + ShortFromChar(&sfpInfo->SwapCrcs[0]);
        if (swapDataLen <= swapDataMaxLen) /*有空间,复制CRC*/
        {
            CommonMemCpy(&swapData[0], swapDataLen, &sfpInfo->SwapCrcs[0], swapDataLen);
        }
        else
        {
...
*返回值:	无
*/
void SfpSetSwapData(UINT8 swapData[], struc_Unify_Info* sfpInfo)
{
    /*PrtclSndBuff格式:4字节总长度(应用和链路)+4字节输出应用长度+N个[2字节长度+1字节协议类型+2字节目的地址+2字节预留+网络数据]+链路数据*/
    SfpSetSwapDataForOut(swapData, &sfpInfo->PrtclSndBuff[8], sfpInfo);
}

/*
*函数名:	SfpSetSwapDataForOut
*功能描述:	设置指定输出数据中双CPU交互数据
*输入参数:	UINT8 swapData[] 交互数据(2字节长度+N个[4字节CRC])
...
*返回值:	无
*/
void SfpSetSwapDataForOut(UINT8 swapData[], UINT8 outData[], struc_Unify_Info* sfpInfo)
{
    ...
    if ((NULL != swapData) && (NULL != outData) && (NULL != sfpInfo)) /*参数检查*/
    {
        record = sfpInfo->RecordArray;
        recordSize = sfpInfo->RecordArraySize;
        PrintRecArrayByte(record, recordSize, 0xE2u);
        ...
        if ((0u < swapDataLen) && (0u == (swapDataLen % 4u))) /*长度合法*/
        {
            ...
            if (0u < allLen) /*有输出*/
            {
                ...
                for (i = 0u; i < dstNum; i++)
                {
                    ...
                    if ((34u <= pkgLen) && ((outAppIdx + pkgLen) <= allLen)) /*包长度大于等于(7字节头+27字节帧格式),且当前输出应用位置加长度小于等于输出应用长度,合法*/
#else
                    if ((30u <= pkgLen) && ((outAppIdx + pkgLen) <= allLen)) /*包长度大于等于(7字节头+23字节帧格式),且当前输出应用位置加长度小于等于输出应用长度,合法*/
#endif
                    {
#ifdef DUAL_CRC
                        if (0u == sfpInfo->CpuId) /*本方是CPU0,设置CPU1 CRC*/
                        {
                            CommonMemCpy(&outAppData[outAppIdx + pkgLen - 4u], 4u, &swapData[2u + 4u * i], 4u);
                        }
                        else /*本方是CPU1,设置CPU0 CRC*/
                        {
                            CommonMemCpy(&outAppData[outAppIdx + pkgLen - 8u], 4u, &swapData[2u + 4u * i], 4u);
                        }
#else
#ifdef SWAP_CRC
                        CommonMemCpy(&outAppData[outAppIdx + pkgLen - 4u], 4u, &swapData[2u + 4u * i], 4u);
