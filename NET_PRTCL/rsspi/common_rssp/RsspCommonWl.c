...
* 2013/02/16    王佩佩  将RSR协议的RSRcommom与RSSPCommom合并
************************************************************************/
...
#define DIMTAB_WL			256

static const UINT32 SYS_CHK_WD_WL[2] ={0xAE390B5AL,0xC103589CL};	/* system check word */
static UINT32 gTableFast_WL[CHAN_NUM_WL][DIMTAB_WL];					/* LFSR快速移位使用的表 */
static const UINT32 gMaskLFSR_WL[CHAN_NUM_WL] = 						/* LFSR移位 MASK表 */
 ...
 * LFSR基本操作函数声明		Start
 *---------------------------------------------------------------------*/
static UINT8 inittab_WL(UINT8 chan);
static UINT32 pd_shift_left_WL(UINT32 regLFSR, UINT8 chan, UINT8 nshift);
static UINT32 pd_shift_right_WL(UINT32 regLFSR, UINT8 chan, UINT8 nshift);
static UINT32 pd_shl_fast_WL(UINT32 regLFSR, UINT8 chan, UINT8 nshift);
...
{
	regLFSR ^= val;
	regLFSR = pd_shl_fast_WL(regLFSR, chan, 32);
	return regLFSR;
}
...
{
	regLFSR = pd_shift_right_WL(regLFSR, chan, 32);
	regLFSR ^= val;
	return regLFSR;
}
...
/***********************************************************************
 * 方法名   : inittab_WL
 * 功能描述 : 对全局LFSR左移常数表gTableFast_WL进行初期化
 * 输入参数 : 
 *	参数名		类型		输入输出		描述
 *  --------------------------------------------------------------
 *	chan		UINT8		in		channel ID
 * 
 * 全局变量 : gTableFast_WL[CHAN_NUM_WL][DIMTAB_WL]
 * 返回值   : UINT8		1:正常		0:错误
 ***********************************************************************/
static UINT8 inittab_WL(UINT8 chan)
{
	...
	for(index = 0; index < DIMTAB_WL; index++) 
	{
		
		value = index << 24;
		value = value & 0x0FF000000;
		regLFSR = F_LOAD_WL(regLFSR,value);
		regLFSR = pd_shift_left_WL(regLFSR,chan,8);
		gTableFast_WL[chan][index] = F_READ_WL(regLFSR);
	}
	ret = 1;
	return ret;
}
...
static UINT32 pd_shl_fast_WL(UINT32 regLFSR, UINT8 chan, UINT8 nshift)
{
	...
	UINT8 shiftCount = 0;
	UINT8 index = 0;
	static UINT8 tabInitFlg[CHAN_NUM_WL] = {0};
	
	/* 对快速左移表初期化 */
	if( 1 != tabInitFlg[chan] )
	{
		tabInitFlg[chan] = inittab_WL(chan);
	}
	...
	for(index = 0; index < shiftCount; index++) 
	{
		...
		regLFSR <<= 8;
		regLFSR &= 0x0FFFFFF00;
		regLFSR ^= gTableFast_WL[chan][tmp&(UINT32)0xFF];
	}
	return regLFSR;
}
...
UINT8 GetSidXorTn_WL(const UINT8* pBsdMsg,UINT32 outSidXorTn[CHAN_NUM_WL])
{
	...
	UINT32 CRC[CHAN_NUM_WL] = {0};		/* 报文数据的安全CRC */
	UINT32 CRCM[CHAN_NUM_WL] = {0};	/* 报文的CRCM */
	...
	dataLen = ShortFromCharLE(&pBsdMsg[RSSP_DATA_SIZE_POS_WL])-2*CRCM_LEN_WL;		/* 数据长度获得 */
	
	for(chan = 0; chan < CHAN_NUM_WL; chan++)
	{
		/* 计算CRC */
		if(0 == dataLen)
		{
		...
		{
			CRC[chan] = CRC_Compute_Crc_WL(&pBsdMsg[RSSP_DATA_POS_WL], dataLen, chan);	/* 计算32位CRC */
		}
		
		CRCM[chan] = LongFromCharLE(&pBsdMsg[RSSP_CRCM_1_POS_WL + chan * CRCM_LEN_WL]);			/* 取得CRCM */
		
		/* 计算 报文的 SID^T(n) */
		outSidXorTn[chan] = (CRCM[chan] ^ CRC[chan] ^ SYS_CHK_WD_WL[chan]);
	}
	return ret;
}
...
UINT8 CheckCRCM_WL(const UINT32 sidXorTn[CHAN_NUM_WL], 
				...
				UINT32 nDelayTCr, 
				const UINT32 postRxdataK[CHAN_NUM_WL][DSURSR_MAX_K_NUM_WL])
{
	UINT32 regLFSR = 0;
	...
	for(chan = 0; chan < CHAN_NUM_WL; chan++)
	{
		...
		regLFSR = F_LOAD_WL(regLFSR, sinitAddSidXorTn[chan]);
		for(index = 0; index < nDelayTCr; index++)
		{
			regLFSR = F_ADD_WL(regLFSR, chan, 0);
		}
		regLFSR = F_ADD_WL(regLFSR,chan,sidXorTn[chan]);
		regLFSR = F_ADD_WL(0,chan,regLFSR);
		
		/* 检查CRCM正确与否 */
		if( F_READ_WL(regLFSR) != postRxdataK[chan][nDelayTCr])
		{
			ret = 0;
			break;
		} 
...
UINT8 CalcCRCM_WL(const UINT8 *pData, UINT16 DataLen, const UINT32 SID[CHAN_NUM_WL], const UINT32 Tn[CHAN_NUM_WL], UINT32 OutCRCM[CHAN_NUM_WL])
{
	...
	else
	{
		...
		for(chan = 0; chan < CHAN_NUM_WL; chan++)
		{		
			crc32[chan] = CRC_Compute_Crc_WL(pData,DataLen,chan);		/* 计算32位安全CRC */
			/* CRCM= CRC32 ^ SYSCKW ^ SID ^ Tn */
			OutCRCM[chan] = crc32[chan] ^ SYS_CHK_WD_WL[chan] ^ SID[chan] ^ Tn[chan];
		}
	}
	ret =1;
	return ret;
 ...
 *	pTmStmpStru_WL		TimeStampStru*	inout	时间戳结构体，更新上次收到报文时间及SINIT ADD (SID ^ Tn)值
 * 
 * 返回值   : UINT8 1：正常返回		0:异常返回
 ***********************************************************************/
UINT8 UpdtTmStmpStru_WL(const UINT32 sidXorTn[CHAN_NUM_WL], 
				UINT32 nowRecvTCr,
				UINT32 nDelayTCr, 
				const UINT32 precSinitK[CHAN_NUM_WL][DSURSR_MAX_K_NUM_WL],
				TimeStampStru_WL* pTmStmpStru)
{
	...
	for(chan = 0; chan < CHAN_NUM_WL; chan++)
	{
		...
		regLFSR = F_LOAD_WL(regLFSR,pTmStmpStru->NowSINIT_ADD_SID_XOR_Tn[chan]);		/* load SINIT ADD (SID ^ T(n-k))  */
		for(index = 0; index < nDelayTCr + 2; index++)
		{
			regLFSR = F_ADD_WL(regLFSR, chan, 0);
		}
		resultA = F_READ_WL(regLFSR);
		
		/* Result_B = PREC_SINIT_K(SIDSENDER) ADD (SID ^ T(n+1)) ADD (SID ^ T(n+1)) */
		regLFSR = F_LOAD_WL(regLFSR, precSinitK[chan][nDelayTCr]);
		regLFSR = F_ADD_WL(regLFSR,chan,sidXorTn[chan]);
		regLFSR = F_ADD_WL(regLFSR,chan,sidXorTn[chan]);
		resultB = F_READ_WL(regLFSR);
				...
				UINT32 outSntAndSidXorTn[CHAN_NUM_WL])
{
	...
	for(chan = 0; chan < CHAN_NUM_WL;chan++)
	{
		regLFSR = F_LOAD_WL(regLFSR,precFirstSinit[chan]);
		regLFSR = F_ADD_WL(regLFSR,chan,senqINI[chan]^enqTCn[chan]);
		outSntAndSidXorTn[chan] = F_READ_WL(regLFSR);
	}
	ret =1;
	return ret;
}
...
UINT8 AddTimeStamp_WL(TimeStampStru_WL *pTmStmp)
{
	...
	else
	{
		...
		for(chan = 0; chan < CHAN_NUM_WL; chan++)
		{
			...
			/* T(n+1) = T(n) ADD 0 */
			regLFSR = F_LOAD_WL(regLFSR, pTmStmp->NowTn[chan]);
			regLFSR = F_ADD_WL(regLFSR, chan, 0);
			pTmStmp->NowTn[chan] = F_READ_WL(regLFSR);
		}
	}
	return ret;		/* 正常返回 */
}
...
UINT32 CRC_Compute_Crc_WL(const UINT8* pData, UINT16 size, UINT8 chan)
{
	...
	{
		Result = (Result >> 8) ^ CrcmTab[chan][(Result ^ Reflected_Byte[pData[offset]]) & 0xFF];
		offset++;
	}
...
{
#ifdef TYPE_ID_2_DEVNAME_USE_COMM_WL
	UINT8 buff[2];
	buff[0] = Type;
	buff[1] = Id;			  
 ...
 * 参数说明： CommonStackWl *pStack,      待初始化的的堆栈
 *            UINT32 TotalSize,         堆栈大小
 * 返回值  ： 1，成功
 *            0，失败
 */
UINT8 StackInitialWl(CommonStackWl* pStack, UINT16 TotalSize)
{
	UINT8 bRet = 0;/*用于函数返回*/
	...
	if ((NULL != pStack) && (0 != TotalSize))
	{
		...
		/*堆栈空间分配*/
		pStack->pStackData = (STACK_DATA_TYPE_WL*)malloc(sizeof(STACK_DATA_TYPE_WL) * (pStack->TotalSize));
...
* 功能描述      : 从堆栈中弹出栈顶元素，
* 输入参数      :
* 输入输出参数  : CommonStackWl *pStack，    堆栈的指针
* 输出参数      : STACK_DATA_TYPE_WL* pData， 弹出的栈顶元素
* 全局变量      :
* 返回值        : 成功返回1，否则返回0
*******************************************************************************************/
UINT8 StackPopWl(CommonStackWl* pStack, STACK_DATA_TYPE_WL* pData)/* %RELAX<MISRA_8_10> */
{
	UINT8 ret;/*用于函数返回*/
...
/******************************************************************************************
* 功能描述      : 向堆栈中压入一个数据
* 输入参数      : STACK_DATA_TYPE_WL Data，    堆栈数据
* 输入输出参数  : CommonStackWl *pStack，     堆栈的指针
* 输出参数      :
* 全局变量      :
* 返回值        : 成功返回1,失败返回0
*******************************************************************************************/
UINT8 StackPushWl(CommonStackWl* pStack, STACK_DATA_TYPE_WL Data)/* %RELAX<MISRA_8_10> */
{
	UINT8 ret;/*用于函数返回*/
...
* 参数说明： CommonStackWl *pStack,       堆栈指针
* 返回值  ： 1，成功
*/
UINT8 StackFreeWl(CommonStackWl* pStack)
{
...
* 返回值  ： 堆栈中元素个数
*/
UINT16 StackStatusWl(CommonStackWl* pStack)
{
	return    pStack->Top;
}
