 ...
 * 链路管理模块内部函数申明 Start
 *--------------------------------------------------------------------*/
static UINT8 InitRsspLnkInfoWl(const DSU_RSSP_LINK_RINFO_STRU_WL* pRecvDsuLnkInf, const DSU_RSSP_LINK_SINFO_STRU_WL* pOutputDsuLnkInf, RsspLinkInfoStru_WL* pRsspLnkInfo, DSU_RSSP_INFO_STRU_WL *pDsuRsspi);
...
static UINT8 CheckRecvDsuLnkInfoWl(UINT16 LocalDevName, const DataNode_WL *pNode, const DSU_RSSP_LINK_RINFO_STRU_WL *pRecvLnkInfo, DSU_RSSP_INFO_STRU_WL *pDsuRsspi);
static UINT8 CheckOutputDsuLnkInfoWl(UINT16 LocalDevName, const DataNode_WL *pNode, const DSU_RSSP_LINK_SINFO_STRU_WL *pOutputLnkInfo, DSU_RSSP_INFO_STRU_WL *pDsuRsspi);
static UINT8 CheckInputAddrWl(UINT32 LocalDevName,UINT16 SrcAddr,const DSU_RSSP_LINK_RINFO_STRU_WL *pRecvLnkInfo);
/*--------------------------------------------------------------------
 * 链路管理模块内部函数申明 End
 *--------------------------------------------------------------------*/
...
UINT8 RsspLnkStatus_WL(UINT8 dstType, UINT8 dstId, UINT16 dstLogicId, RSSP_INFO_STRU *pRsspi)
{
	...
	Lnk_Info_Node_Stru_WL *pNode = NULL;	/*链路节点*/
	UINT16 dstName = 0U;
	if (NULL != pRsspi)
	{
...
#else
		oppId = DquToHlhtId(dstType, dstId, dstLogicId, pRsspi->pDsuRsspInfoStruWL);	/*获取目的RSSPI唯一ID*/
#endif
		
		pNode = SearchNodeByAddr_WL(oppId, pRsspi);
	}
	
	if (NULL != pNode)
	{
		if (RecvData_WL == pNode->RsspLinInfo.RecvLinkStatus)
		{
			rtn = 0x17;	/*接收数据状态*/
		}
		else
...
UINT8 RsspLnkDelete_WL(UINT32 destDevName,RSSP_INFO_STRU *pRsspStruct)
{
	...
	{
		/*清空链路信息*/
		RsspMemSet_WL((UINT8*)pNode, sizeof(Lnk_Info_Node_Stru_WL), 0);
		/* 通信节点的状态设为 可覆盖状态 */
...
{	 
	...
	{
		RsspMemSet_WL((UINT8*)pRsspStruct->LnkNodeArr, sizeof(Lnk_Info_Node_Stru_WL)*pRsspStruct->LnkNodeNum, 0);
		/* 通信节点设置为可覆盖节点 */
		for (index = 0; index < pRsspStruct->LnkNodeNum; index++)
		{
			pRsspStruct->LnkNodeArr[index].NodeStatus = Replace_WL;
		}
	}
	else
	{
	...
	if( NodeNum <= pRsspStruct->LnkNodeNum)
	{
		...
		{
			/* 通信节点基本信息设定 */
			VarArray2LnkNodeInfo_WL(&VarArray[offset],&pRsspStruct->LnkNodeArr[index]);
			...
			{			
				...
				ret =   dsuRsspRGetLnkInfoWl(pRsspStruct->LnkNodeArr[index].RsspLinInfo.RsspDestAddr,
										...
										&recvLnkInfo, pRsspStruct->pDsuRsspInfoStruWL);
				if(ret == 0)
				{
					/* 数据有误，错误返回 */
					return ret;
...
UINT8 InitLnkNodeArr_WL(UINT8 num,RSSP_INFO_STRU *pRsspStruct)
{
	...
	for(index = 0;index < pRsspStruct->LnkNodeNum;index++)
	{
		...
		/*保持状态周期数清零*/
		pRsspStruct->LnkNodeArr[index].KeepSendSseOrHaltCycleNum = 0;
	}
...
Lnk_Info_Node_Stru_WL* SearchLnkBsdNode_WL(UINT16 RsspSrcAddr,UINT16 DestBsdAdd,RSSP_INFO_STRU *pRsspStruct)
{
	...
	{	
		...
		/* 根据BSD地址和源地址查询 */
		if((DisbaledReplace_WL == pRsspStruct->LnkNodeArr[index].NodeStatus)
			&& (pRsspStruct->LnkNodeArr[index].RsspLinInfo.RsspSrcAddr == DestBsdAdd)
			&& (pRsspStruct->LnkNodeArr[index].RsspLinInfo.RsspDestAddr== RsspSrcAddr))
		{
...
Lnk_Info_Node_Stru_WL* SearchLnkSse_SsrNode_WL(UINT16 RsspSrcAddr,UINT16 RsspDestAddr,RSSP_INFO_STRU *pRsspStruct)
{
	...
	for(index = 0; index < pRsspStruct->LnkNodeNum; index++)
	{
		...
		if((DisbaledReplace_WL == pRsspStruct->LnkNodeArr[index].NodeStatus)
			&& (pRsspStruct->LnkNodeArr[index].RsspLinInfo.RsspDestAddr == RsspDestAddr)
			&& (pRsspStruct->LnkNodeArr[index].RsspLinInfo.RsspSrcAddr == RsspSrcAddr))
		{
			retNodePtr = & pRsspStruct->LnkNodeArr[index];
			break;
		}
 ...
 *
 ***********************************************************************/
UINT8 recvReplaceLnkNode_WL(UINT16 LocalDevName,const DataNode_WL* pDataNode,Lnk_Info_Node_Stru_WL *pLnkNodeInfo, DSU_RSSP_INFO_STRU_WL *pDsuRsrInfoStru_WL)
{
	...
	ret = dsuRsspRGetLnkInfoWl(DestSrcAddr, 
			BsdAddr, &recvLnkInfo, pDsuRsrInfoStru_WL);
	/* 取得成功 而且 基本信息和临时报文节点信息一致 */
	checkRet = CheckRecvDsuLnkInfoWl(LocalDevName, pDataNode, &recvLnkInfo, pDsuRsrInfoStru_WL);
	if((0 == ret) || (0 == checkRet))
	{
		ret = 0;
		return ret;
	...
	ret = dsuRsspSGetLnkInfoWl(recvLnkInfo.SseAddr, (INT16)DestSrcAddr, &outputLnkInfo, pDsuRsrInfoStru_WL);
	/* 取得成功 而且 基本信息和临时报文节点信息一致 */
	checkRet = CheckOutputDsuLnkInfoWl(LocalDevName, pDataNode, &outputLnkInfo, pDsuRsrInfoStru_WL);
	if((0 == ret) || (0 == checkRet))
	{
		ret = 0;
		return ret;
	...
	/* RSSP链路基本信息结构体初始化 */
	InitRsspLnkInfoWl(&recvLnkInfo, &outputLnkInfo, &pLnkNodeInfo->RsspLinInfo, pDsuRsrInfoStru_WL);
...
UINT8 outputReplaceLnkNode_WL(UINT16 SrcAddr,UINT16 BsdAddr,UINT16 DestAddr,
						   UINT32 LocalDevName,Lnk_Info_Node_Stru_WL **pPLnkNodeInfo,
						   RSSP_INFO_STRU *pRsspStruct)
{
	...
	ret = dsuRsspRGetLnkInfoWl(DestAddr, SrcAddr, &recvLnkInfo, pRsspStruct->pDsuRsspInfoStruWL);
	/* 
	 * 取得成功 而且 接受信息与本方信息一致
	 */
	...
	{
		/* 根据取得的Sse地址以及目标地址(BSD地址)取得本方作为发送节点的信息 */
		ret = dsuRsspSGetLnkInfoWl(recvLnkInfo.SseAddr, BsdAddr, &outputLnkInfo, pRsspStruct->pDsuRsspInfoStruWL);
		if(0 == ret)
		{
			/* 取得失败，错误返回 */
			return ret;
		...
		/* RSSP链路基本信息结构体初始化 */
		InitRsspLnkInfoWl(&recvLnkInfo, &outputLnkInfo, &pLnkNodeInfo->RsspLinInfo, pRsspStruct->pDsuRsspInfoStruWL);
 ...
 *	参数名		类型	输入输出	描述
 *  --------------------------------------------------------------------------
 *	destName		UINT32	 IN		对方设备标识
 *
 * 返回值   :	Lnk_Info_Node_Stru_WL* 通信节点指针， NULL:不存在
 ***********************************************************************/
Lnk_Info_Node_Stru_WL* SearchNodeByAddr_WL(UINT32 destName,RSSP_INFO_STRU *pRsspStruct)
{
	UINT8 index = 0;
	...
	for( index = 0; index < pRsspStruct->LnkNodeNum; index++)
	{
		 ...
		 */
    if((DisbaledReplace_WL == pRsspStruct->LnkNodeArr[index].NodeStatus)
		&& (destName == pRsspStruct->LnkNodeArr[index].DestDevName))
		{
...
static UINT8 InitRsspLnkInfoWl(const DSU_RSSP_LINK_RINFO_STRU_WL* pRecvDsuLnkInf, 
							...
							DSU_RSSP_INFO_STRU_WL *pDsuRsspi)
{
	UINT8 ret = 0;
...
#ifdef RSSP_NO_HLHT
		pRsspLnkInfo->DestLogId = pOutputDsuLnkInf->DestName & 0xff;
		pRsspLnkInfo->LocalLogId = pRecvDsuLnkInf->DestName & 0xff;
#else
		pRsspLnkInfo->DestLogId = DquToDevId(pOutputDsuLnkInf->DestName, pDsuRsspi);	/* 对方动态Id （对方设备Id）*/
		pRsspLnkInfo->LocalLogId = DquToDevId(pRecvDsuLnkInf->DestName, pDsuRsspi);	/* 接受基本信息的对方标识符为本方标识符，设备Id作为动态Id */
#endif	
...
UINT8 WriteToVarArray_WL(UINT8 *VarArray,RSSP_INFO_STRU *pRsspStruct)
{
	...
	UINT8 count = 0;
	UINT16 offset = 0U;
	if ((NULL!= VarArray)&&(NULL!= pRsspStruct))
	{
		...
		for (index = 0; index < pRsspStruct->LnkNodeNum; index++)
		{
			...
			if (pRsspStruct->LnkNodeArr[index].NodeStatus == DisbaledReplace_WL)
			{
				...
				/* 当前节点的信息写入中间变量数组 */
				LnkNodeInfo2VarArray_WL(&pRsspStruct->LnkNodeArr[index], &pTmp[offset]);
				/* 下一个节点 */
				offset += NodeLen;
				count++;
			}
		...
		ShortToChar((count * NodeLen), VarArray);
		ret = 1;
	}
...
static void LnkNodeInfo2VarArray_WL(const Lnk_Info_Node_Stru_WL* pNode, UINT8* VarArray)
{
	...
	/*保持状态周期数 1字节*/
	VarArray[index++] = pNode->KeepSendSseOrHaltCycleNum;

	/*4字节对方唯一ID*/
	LongToChar(pNode->DestDevName, &VarArray[index]);
	index += 4;

	/*2字节对方类型ID*/
	ShortToChar(pNode->DestTypeId, &VarArray[index]);
	index += 2;

	/*2字节对方逻辑ID*/
	ShortToChar(pNode->DestLogicId, &VarArray[index]);
	index += 2;
...
#ifndef RSSPI_SIMPLE_LINK_REFRESH
...
static void VarArray2LnkNodeInfo_WL(const UINT8* VarArray, Lnk_Info_Node_Stru_WL* pNode)
{
	...
	/*保持状态周期数 1字节*/
	pNode->KeepSendSseOrHaltCycleNum = VarArray[index++];

	/*4字节对方唯一ID*/
	pNode->DestDevName = LongFromChar(&VarArray[index]);
	index += 4;

	/*2字节对方类型ID*/
	pNode->DestTypeId = ShortFromChar(&VarArray[index]);
	index += 2;

	/*2字节对方逻辑ID*/
	pNode->DestLogicId = ShortFromChar(&VarArray[index]);
	index += 2;
 ...
 /*
*函数名:	CheckRecvDsuLnkInfoWl
*功能描述:	根据DSU取得接收方信息，检查与队列里的信息是否一致
*输入参数:	UINT16 LocalDevName	本方类型ID
*			const DataNode_WL *pNode	收到数据节点
*			const DSU_RSSP_LINK_RINFO_STRU_WL *pRecvLnkInfo	接收方信息
*			DSU_RSSP_INFO_STRU_WL *pDsuRsspi	RSSPI配置信息
*输入出参:	无
*输出参数:	无
*返回值:	1:成功,0:失败
*/
 ...
 {
	 UINT8 rtn = 0;	/*返回值*/
	 UINT8 locType = 0;	/*本方类型*/
	 UINT8 locId = 0;	/*本方ID*/

	 if (NULL != pRecvLnkInfo)	/*输入参非空*/
	 {
		 DevName2TypeId_WL(LocalDevName, &locType, &locId);
		 if ((pRecvLnkInfo->DestName == pDsuRsspi->LocEmitterName) && (pRecvLnkInfo->SseAddr == pDsuRsspi->LocRsspiAddr) && (pNode->LocalLogId == locId))
		 {
			 /*收到数据对应的本方唯一ID正确*/
			 rtn = 1;
		 }
 ...
 /*
*函数名:	CheckOutputDsuLnkInfoWl
*功能描述:	根据DSU取得发送方信息，检查与队列里的信息是否一致
*输入参数:	UINT16 LocalDevName	本方类型ID
*			const DataNode_WL *pNode	收到数据节点
*			const DSU_RSSP_LINK_SINFO_STRU_WL *pOutputLnkInfo 发送方信息
*			DSU_RSSP_INFO_STRU_WL *pDsuRsspi	RSSPI配置信息
*输入出参:	无
*输出参数:	无
*返回值:	1:成功,0:失败
*/
 static UINT8 CheckOutputDsuLnkInfoWl(UINT16 LocalDevName, const DataNode_WL *pNode, const DSU_RSSP_LINK_SINFO_STRU_WL *pOutputLnkInfo, DSU_RSSP_INFO_STRU_WL *pDsuRsspi)
 {
	 ...
	 if (NULL != pOutputLnkInfo)	/*输入参非空*/
	 {
		 DevName2TypeId_WL(LocalDevName, &locType, &locId);	/*本方设备ID*/
#ifdef RSSP_NO_HLHT
		 dstId = pOutputLnkInfo->DestName & 0xff;
#else
		 dstId = DquToDevId(pOutputLnkInfo->DestName, pDsuRsspi);	/*对方设备ID*/
#endif

		 if ((pOutputLnkInfo->DestName == pNode->DevName) && (pNode->LocalLogId == locId) && (pNode->DestLogId == dstId))
		 {
			 /*收到数据对应的对方设备ID正确*/
			 rtn = 1;
		 }
 ...
 * 返回值   : UINT8  1：正常返回  0：错误返回
 ***********************************************************************/
static UINT8 CheckInputAddrWl(UINT32 LocalDevName,UINT16 SrcAddr,
						  ...
						  NodeParamInfoStru_WL *pNodeParam)
{
	...
	else
	{
		for(chan = 0; chan < 2; chan++)
		{
			...
			for( index = 0; index < DSURSR_MAX_K_NUM_WL; index++)
			{
...
void AdjustLnkCycle(UINT8 localType, UINT32 cycleNum, Lnk_Info_Node_Stru_WL *lnkNode)
{
	if (lnkNode != NULL)
	{
		if ((RSSP_TYPE_VOBC_WL == localType) && (RSSP_TYPE_ATS_WL == (lnkNode->DestTypeId / 256)))	/*VOBC发往ATS*/
		{
		...
		else if ((RSSP_TYPE_AOM_WL == localType) && (RSSP_TYPE_ATS_WL == (lnkNode->DestTypeId / 256)))	/*AOM发往ATS*/
		{
