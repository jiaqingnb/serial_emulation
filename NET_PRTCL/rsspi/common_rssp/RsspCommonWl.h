...
#ifndef	RSSP_COMMONWL_H
#define	RSSP_COMMONWL_H
...
#define RSSP_TYPE_TTIL_WL   0x17	/*列尾*/
...
#define RSSPI_TYPE_IN_ADAPTER (0x04) /*RSSPI在适配层中类型*/
#define SVC_ARRAY_MAX_LEN ((UINT16)(4*255)) /*最大255个链路的*/
...
#define RP_LINK_LEN (14)	/*冗余层链路长度*/

#ifdef RSSPI_SIMPLE_LINK_REFRESH
#define RSSP_VAR_NODE_LEN_WL (56) /*简单链路刷新,中间变量数组一个节点的长度*/
#else
#define RSSP_VAR_NODE_LEN_WL (85) /*中间变量数组一个节点的长度*/
#endif
...
typedef void* STACK_DATA_TYPE_WL;

/*堆栈结构体定义*/
typedef struct
{
	void** pStackData;         /*空闲节点值，表示的是存储数据的空间*/
...
* 返回值        : 成功返回1，否则返回0
*******************************************************************************************/
UINT8 StackPopWl(CommonStackWl* pStack, STACK_DATA_TYPE_WL* pData);
...
* 返回值        : 成功返回1,失败返回0
*******************************************************************************************/
UINT8 StackPushWl(CommonStackWl* pStack, STACK_DATA_TYPE_WL Data);
