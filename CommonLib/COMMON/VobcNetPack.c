...
*  返回值：   打包后信息的数据长度
*  修改记录： 修复产品白盒集成测试缺陷Bug-2012,hc,20200218
*/
UINT16 VobcNetPack(IN UINT8 DataType,IN UINT32 Seq,IN const UINT8 *pData, IN UINT16 DataLen,OUT UINT8 *pPackData)
{
	...
	if((NULL != pData) && (NULL != pPackData))
	{
		/*数据赋值*/
		PackDataLen = DataLen + 11;
		...
		*pPack=DataType;
		...
		/*CRC32*/
		CRCValue=Crc32(pPackData,(UINT32)(PackDataLen-4));
...
UINT16 VobcNetUnpack(IN const UINT8 *pPackData,IN const UINT16 PackDataLen,OUT UINT8 *pDataType,OUT UINT32 *pSeq,OUT UINT8 *pData)
{
    ...
    UINT32 PackCRCValue = 0;
    UINT32 CalcCRCValue = 0;
    UINT16 tempLen = 0;
    UINT16 i = 0;
    ...
    /*判断数据长度是否正确*/
    if ((NULL != pPackData) && (NULL != pDataType) && (NULL != pSeq) && (NULL != pData)
		&& ((tempLen+2)==PackDataLen))
    { 
        /*自己计算得到的CRC32*/
        tempLen=PackDataLen-4;
        CalcCRCValue=Crc32(pPackData,(UINT32)tempLen);

        /*打包数据中取出的crc32*/
        PackCRCValue=LongFromChar((const UINT8 *)(pPackData+PackDataLen-4));
